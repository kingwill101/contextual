...
`HttpLogger` captures elapsed time, memory usage, and process ID (`contextual_shelf/lib/src/http_logger.dart:7-57`). Use channel-specific middlewares or the new [Type Formatters & Printers](type-formatters.mdx) guide to format these values consistently.
